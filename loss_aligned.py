...
def global_loss(tri_loss, global_feat, labels, normalize_feature=True,scale = 1.0):
  """
  Args:
  ...
  if normalize_feature:
    global_feat = normalize(global_feat, axis=-1)*scale
    #print(scale)    # shape [N, N]
...
def global_loss_cosine(tri_loss, global_feat, labels):
  """
  Args:
    tri_loss: a `TripletLoss` object
    global_feat: pytorch Variable, shape [N, C]
  ...
  array1 = normalize(global_feat, axis=-1)
  dist = torch.mm(array1, array1.t())
  dist_mat = 1.0 - dist
  dist_ap, dist_an, p_inds, n_inds = hard_example_mining(
    dist_mat, labels, return_inds=True)
  loss = tri_loss(dist_ap, dist_an)
...
def local_loss(
  ...
  """
  if normalize_feature:
    local_feat = normalize(local_feat, axis=-1)*scale
